...
/*
 * A module is a list of nodes within a reference network. self.divide(nobj) returns delta_Q and
 * [group1, group2]. If the division is not successful, returned groups are empty. The reference
 * network is passed in via argument nobj.
 */
public class Module {
  ...
  Module(List<String> nodes) {
    ...
    /*
     * Keep track of nodes by their names (ids) because the indexes change from network matrix to
     * subnet. node names -> node index via reference_network.crd
     */
  ...
  List<List<String>> eigenDivide(Network networkobj) {
    /*
     * divide module in two by leading eigenvector. This function reproduced the karate club data as
     * given in http://www-personal.umich.edu/~mejn/courses/2004/cscs535/problems5.pdf
     */
  ...
  Double computeDeltaQ(Network networkobj) {
    /*
     * compute gain in modularity, delta_Q, by Eq.[5]. In Newman's paper, this step is suggested to
     * be bypassed by Eq.[7]. However, Eq.[5] can be written as (1/4m) * SUM[(s_i*s_j - 1)*B_(i,j)],
     * and using B and B.sum() is more convinient in Numpy. I have not evaluated how my approach
     * affects speed of the algorithm, though matrix operations in Numpy are respectable.
     */
    double dQ = 0.0;
    for (int i = 0; i < this.numOfNodes; i++) {
  ...
  void fetchModularityMatrix(Network networkobj) {
    ...
    for (int k = 0; k < this.numOfNodes; k++) {
      for (int l = 0; l < this.numOfNodes; l++) {
        am[k][l] =
            networkobj.getModularityMatrix()[networkobj.getCrd().get(this.nodes.get(k))][networkobj
