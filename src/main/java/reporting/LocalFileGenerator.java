...
public class LocalFileGenerator {
  ...
  public void exportModularAnalysisResult(String filePath) {
    ...
    try {
      ...
      data.add(new String[] {"Module", "P-Value", "Size", "Members(Name)", "Memebers(ID)",
      ...
      for (MModule mm : modularAnalaysis.getTopModules()) {
        ...
        data.add(new String[] {"module_" + counter, String.valueOf(mm.getpValue()),
            String.valueOf(nodes.size()), create_Names(nodes), create_Nodes(nodes),
            findTopPathways(nodes)});
      }
  ...
  String findTopPathways(List<String> nodes) {
    ...
    for (String node : nodes) {
      try {
        pathways
            .addAll(this.mixedNetowrk.getModel().getMetabolicModel().getCpd2pathways().get(node));
      } catch (Exception e) {
        LOGGER.error(e.getMessage());
        continue;
      }
  ...
  String create_Names(List<String> nodes) {
    ...
    for (String node : nodes) {
      try {
        result.append(this.mixedNetowrk.getModel().getMetabolicModel().getDict_cpds_def().get(node))
            .append(";");
      } catch (Exception e) {
        result.append(";");
      }
  ...
  String create_CmpdsNames(List<List<String>> compunds) {
    ...
    for (List<String> li : compunds) {
      for (String cmpd : li) {
        try {
          result
              .append(this.mixedNetowrk.getModel().getMetabolicModel().getDict_cpds_def().get(cmpd))
              .append(";");
        } catch (Exception e) {
  ...
  String createCompundNames(EmpiricalCompound ec) {
    ...
    for (String s : ec.getCompounds()) {
      try {
        result.append("\"")
            .append(this.mixedNetowrk.getModel().getMetabolicModel().getDict_cpds_def().get(s))
            .append("\"").append(";");
      } catch (Exception e) {
        result.append("").append(";");
      }
