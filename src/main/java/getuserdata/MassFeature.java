...
/*
 * Data model, to store info per input feature row_number is used as unique ID. A string like
 * "row23" is used instead of integer for two reasons: to enforce unique IDs in string not in
 * number, and for clarity throughout the code; to have human friendly numbering, starting from 1
 * not 0
 */
...
public class MassFeature {
  ...
  MassFeature(String row_number, Double mz, Double retention_time, Double p_value, Double statistic,
  ...
  String make_str_output() {
    ...
    return "\t" + this.row_number + this.mz + this.retention_time + this.p_value + this.statistic
        + this.compoundID_from_user;
  ...
  public int hashCode() {
    ...
    result = prime * result
        + ((matched_EmpiricalCompounds == null) ? 0 : matched_EmpiricalCompounds.hashCode());
