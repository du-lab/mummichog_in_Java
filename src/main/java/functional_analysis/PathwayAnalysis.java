...
public class PathwayAnalysis {
	...
	public void cpd_enrich_test() {
		...
		for(MetabolicPathway mp: this.pathways) {
			...
			if(overlapSize>0) {
				...
				//TODO Perform Fisher Exact Test
				//p_FET = stats.fisher_exact([[overlap_size, query_set_size - overlap_size],
                //[ecpd_num - overlap_size, negneg]], 'greater')[1]
				mp.setpFet(FisherExact.getInstance(20000).getRightTailedP(overlapSize, (query_set_size-overlapSize), (epcdNum-overlapSize), nonneg));
	...
	public List<Double> calculate_permutation_value(List<EmpiricalCompound> queryEmpriricalCompunds , List<MetabolicPathway> pathways) {
		...
		for(MetabolicPathway mp: pathways) {
			...
			overlapCompunds=Sets.intersection(new HashSet<EmpiricalCompound>( queryEmpriricalCompunds), mp.getEmpiricalCompounds());		
			...
			if(overlapSize>0) {
                ...
                //[ecpd_num - overlap_size, negneg]], 'greater')[1]
				//double pval=0;
				result.add(FisherExact.getInstance(20000).getRightTailedP(overlapSize, (querySetSize-overlapSize), (epcdNum-overlapSize), nonneg));
			}else {
				result.add(1.0);
			}
