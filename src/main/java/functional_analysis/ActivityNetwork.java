...
public class ActivityNetwork {
	...
	public ActivityNetwork(List<RowEmpcpd> hitTriosList, DataMeetModel mixedNetwork) {
		/*
		 * Build a consensus network for hit_Trios, [(mzFeature, EmpiricalCompound,
		 * cpd),...] for top_modules and sig pathways. hit_Trios =
		 * set(PA.collect_hit_Trios() + MA.collect_hit_Trios())
		 */
	...
	public Graph<String, DefaultEdge> build_Activity_Network(List<String> nodes) {
		...
		/*
		 * Get a network with good connections in no more than 3 steps. No modularity
		 * requirement for 1 step connected nodes.
		 */

		int expected_size = 10;

		Graph<String, DefaultEdge> an = new AsSubgraph<String, DefaultEdge>(this.mixedNetwork.getModel().getNetwork(),
	...
	public Graph<String, DefaultEdge> getLargestSubgraph(Graph<String, DefaultEdge> an) {
		...
		List<Graph<String, DefaultEdge>> connectedComponents = new ArrayList<Graph<String, DefaultEdge>>(
